 ...
 * @access  Public
 *
 * @queryParam {string} ids - Comma-separated list of exercise IDs.
 *
 * @response
 * - 200: Returns an array of exercise objects (populated with muscle data).
 * - 400: Invalid exercise ID format.
 * - 404: One or more exercises not found.
 * - 500: Internal server error.
 */
exports.get_exercises_by_ids = async (req, res) => {
    try {
        const { ids } = req.query;
        if (!ids) {
        ...
        for (let id of idsArray) {
            if (!mongoose.Types.ObjectId.isValid(id)) {
                return res.status(400).json({ error: `Invalid exercise ID: ${id}` });
            }
        }

        // Fetch exercises that match the provided IDs
 ...
 * @desc    Fetch alternative exercises based on a specified primary muscle.
 *          Returns up to 5 exercises that target the provided primary muscle,
 *          excluding the current exercise if its ID is provided.
 * @access  Public
 *
 * Query Parameters:
 * - primaryMuscle: The primary muscle to filter exercises by (expects an ObjectId or string).
 * - currentExerciseId: The ID of the current exercise to exclude from the results.
 * - limit: Number of exercises to return (default: 5, maximum: 5).
 *
 * @response
 * - 200: Returns an object containing an array of alternative exercises.
 * - 400: Bad request (e.g., missing primaryMuscle parameter).
 * - 500: Internal server error.
 */
exports.get_alternative_exercises = async (req, res) => {
    try {
        // Ensure the required primaryMuscle parameter is provided.
        const primaryMuscle = req.query.primaryMuscle;
        if (!primaryMuscle) {
            return res.status(400).json({ error: "Missing required parameter: primaryMuscle" });
        }

        // Optionally get the current exercise ID to exclude it from results.
        const currentExerciseId = req.query.currentExerciseId;

        // Parse limit parameter; default to 5 and enforce a maximum of 5.
        let limit = parseInt(req.query.limit) || 5;
        if (limit > 5) {
            limit = 5;
        }

        // Build the filter query: match primaryMuscle and optionally exclude the current exercise.
        const filterQuery = { primaryMuscles: primaryMuscle };
        if (currentExerciseId) {
            filterQuery._id = { $ne: currentExerciseId };
        }

        // Find exercises that match the filter.
        const alternatives = await Exercise.find(filterQuery)
            .populate("primaryMuscles secondaryMuscles")
            .limit(limit);
